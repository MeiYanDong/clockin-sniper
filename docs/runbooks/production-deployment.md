...
This runbook describes a reproducible deployment, but it is not a deployment receipt. The 2026-08-20 verified CLOCKIN primary path is a WETH quoted launch pad: exact pad plus approved creator `LaunchCreated` discovers and authorizes the CA/id, same-id `LaunchArmed` completes launch configuration, and the Executor then reads dynamic tax/launch/quote state before calling `buy(id, quoteIn, minTokensOut, ref)`. Name, symbol, X and website observations are asynchronous audit inputs only; they never authorize or veto the creator-first hot path.

The old “final mainnet Launcher Factory/launch/buy ABI unpublished” blocker is historical and has been superseded for quoted entry. The official 2026-08-20 launch was missed because revision 17 let an exact token-name comparison veto an already-matched Factory and approved creator; both curves have since graduated. The creator-first v3 source and release artifact are new, while the prior wallet-preparation receipt and authorization are historical and expired. The current safe production state keeps only keyless Control active, paid services/path stopped and both markers absent. A future one-shot authorization requires a current 10/10 WETH/allowance/native-Gas/nonce readback; an old preparation receipt cannot be inherited.

Do not claim `ENTRY_HOT_ARMED` until a reviewed receipt proves the quoted artifact/profile/one-shot authorization are installed, the wallet preparation has canonical receipts, all ten wallets have current WETH/allowance/native-Gas/nonce readiness, and `clockin-executor.path` is the sole enabled paid-side trigger. Before a handoff, Executor and Reconciler must remain inactive; their inactivity is expected and proves that Chainstack is not being consumed early. For creator-first v3, all ten lanes require strong Factory+creator binding, fresh Reconciler/readiness and positive bounded quotes at the first current-tax `<=5000 bps` block; they do not wait for a canary receipt. Generic quoted Exit remains unsupported and is not an entry prerequisite or an automatic-exit claim.
...
8. The public Control plane accepts only exact-pad, exact-approved-creator, `externalToken=false`, canonical `LaunchCreated` logs and writes the non-sensitive handoff before token metadata audit. Name/symbol mismatch, timeout or ABI failure is recorded asynchronously and cannot veto the handoff. The valid-only signal starts the paid Executor, which in turn starts its required Reconciler. Both processes revalidate the two markers at runtime.
